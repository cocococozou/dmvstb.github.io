...
With an average following-up period of `r (mean(df_descrip$days)/365) %>% round(3)` year , `r (sum(df_descrip$tb)) %>% round(3)` TB cases were recorded among `r nrow(df_descrip)` T2DM patients from `r (sum(df_descrip$days)/365) %>% round(3)` person-years follow-up. The incident rate of TB was `r (100000*(sum(df_male$tb)/(sum(df_male$days)/365))) %>% round(3)` per 100 000 person-years for men, and `r (100000*(sum(df_female$tb)/(sum(df_female$days)/365))) %>% round(3)` per 100 000 person-years for women.
